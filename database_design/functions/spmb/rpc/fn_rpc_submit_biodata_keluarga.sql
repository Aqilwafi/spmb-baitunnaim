create or replace function public.fn_rpc_submit_biodata_keluarga(
  ...
  p_nama_lengkap        varchar(150) default null,
  ...
  p_is_skipped          boolean default false -- Parameter penanda dilewati
)
returns jsonb
language plpgsql
volatile
...
begin
  if v_owner_user_id is null then
    raise exception 'Unauthorized: user tidak terautentikasi' using errcode = '28000';
  end if;
  ...
  -- Validasi step linear dijalankan terlebih dahulu
  v_next_step := public.fn_assert_linear_step(p_form_id => p_form_id);
  ...
  if v_biodata_siswa_id is null then
    raise exception 'Formulir tidak ditemukan atau Anda tidak memiliki akses' using errcode = 'P0002';
  end if;

  -- JIKA WALI DILEWATI (SKIPPED): Lewati proses validasi detail wali dan abaikan insert tabel keluarga
  if p_relation_type = 'WALI' and p_is_skipped = true then
    -- Hanya update step pendaftaran saja
  ...
  -- (Lanjutkan logika validasi normal, insert/upsert seperti biasa di bawah sini...)
  if p_relation_type not in ('AYAH', 'IBU', 'WALI') then
    raise exception 'relation_type harus salah satu dari AYAH, IBU, WALI' using errcode = 'BN400';
  end if;

  if p_relation_type = 'WALI' then
    if p_status_hidup != 'HIDUP' then
      raise exception 'WALI harus hidup' using errcode = 'BN400';
    end if;
    if p_detail_relation_type is null or trim(p_detail_relation_type) = '' then
      raise exception 'WALI harus memiliki detail_relation_type' using errcode = 'BN400';
    end if;
    if p_nik is null or trim(p_nik) = '' then
      raise exception 'WALI harus memiliki NIK' using errcode = 'BN400';
    end if;
    if p_no_hp is null or trim(p_no_hp) = '' then
      raise exception 'WALI harus memiliki nomor HP' using errcode = 'BN400';
    end if;
    if p_nama_lengkap is null or trim(p_nama_lengkap) = '' then
      raise exception 'WALI harus memiliki nama lengkap' using errcode = 'BN400';
    end if;
  end if;
...
exception
  when unique_violation then
    raise exception 'NIK sudah terdaftar untuk data keluarga lain' using errcode = '23505';
  when check_violation then
    raise exception 'Data tidak lengkap atau tidak valid untuk kombinasi status_hidup/relation_type ini' using errcode = '23514';
end;
$$;