...
const isPanitia = (roles: (string | number)[]) => roles.includes(4);
...
export const hasSpmbAccess = (roles: (string | number)[]) =>
  isPanitia(roles) || isHighAdminLevel(roles);

export const hasPublikasiAccess = (roles: (string | number)[]) =>
  isPublikator(roles) || isHighAdminLevel(roles);

export const hasManageAccess = (roles: (string | number)[]) => 
  isHighAdminLevel(roles);