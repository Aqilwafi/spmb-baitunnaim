...
interface SiswaTableProps extends BiodataClientProps {
  ...
  onSave?: (siswa: any) => void;
}

export function BiodataSiswaTable({ data, onViewDetail, onEdit }: SiswaTableProps) {
  // Menentukan lebar kolom aksi berdasarkan tombol yang aktif
  const hasActions = Boolean(onViewDetail || onEdit);
      ...
      <div className="overflow-x-auto overflow-y-auto max-h-[500px] relative">
        <Table className="w-full text-left border-collapse">
          <TableHead className="sticky top-0 z-10 bg-gray-50 border-b border-gray-200 text-xs font-semibold text-gray-600 uppercase tracking-wider shadow-sm">
            <TableRow>
              <TableHeader className="p-4 w-[28%] bg-gray-50">Nama Lengkap</TableHeader>
              ...
              {hasActions && <TableHeader className="p-4 w-[10%] bg-gray-50 text-center">Aksi</TableHeader>}
            </TableRow>
          </TableHead>
          <TableBody className="divide-y divide-gray-200 text-xs text-gray-700">
            {data && data.length > 0 ? (
              data.map((siswa: any) => (
              ...
              <TableRow>
                <TableCell colSpan={hasActions ? 6 : 5} className="p-8 text-center text-gray-500">
